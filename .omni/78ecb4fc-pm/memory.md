...
- 線上投票提交草稿在 `personal-auto/online-voting-submission.md`；2026-07-27 依 Sofia 回饋「不夠吸睛」全面重製為 v2 三版：v2-A 3D 吉祥物、v2-B 普普爆點海報、v2-C 電影感實景（檔名 `vote-submit-v2-*-768x514.png`），底圖用 gpt-image-1 生成、繁中文字以 Noto Sans TC 程式合成避免 AI 錯字；建議主投 v2-A。2026-07-27 已將 Cloudflare Pages `https://from-carbon-metro-vote.pages.dev/` 改成 v2-A 風格民眾投票展示頁，互動 prototype 在同站內 `/demo/`；已全面地毯式清除展示頁的內部/開發開發人員語言。為提升首頁首屏加載速度，已將主圖優化為 50.8 KB WebP（縮減 90%）並採用 WebP 預載，且 PNG 備份也壓縮至 123.8 KB。後續將展示頁升級：使用 Numbers 官方 Favicon 與暗色背景 Brightness Invert 標誌，新增自製精美 inline SVG「捷運點」金幣與綠葉標章，並加入流暢的 CSS 卡片 Neo-brutalism 浮動、商標 360 度旋轉及技術徽章脈動等普普風小動畫，並通過手機移動端高靈敏與完美響應式排版驗證。公開 URL、手機 UA、優化後主圖、agent-readiness 檔、離線備援包與 demo smoke test 均通過。GitHub Pages production 仍是舊版，`from-carbon-to-metro` 本地分支 ahead，且 GitHub API 認證回 401。
...
---
_Last system refresh: 2026-07-27 10:09 UTC_
